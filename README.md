...
└── LeastSquareProny                   overdetermined_prony 初猜 + 梯度下降精化（仅复数）
```
...
| `DeterminedProny` | 精确 Prony（仅用前 2p 个点，`determined_prony`） | 实数 / 复数 |
| `MatrixPencil` | 矩阵束法（Hankel 矩阵 SVD + 广义特征值，`matrix_pencil`） | 实数 / 复数 |
| `LeastSquareProny` | overdetermined_prony 初猜，再以 `(模,相位)` 实参数化做梯度下降（`lsq_expansion_n`） | 复数 |

底层可直接调用的求解函数：`determined_prony`、`overdetermined_prony`、`matrix_pencil`、`lsq_expansion_n`。
...
测试按单元拆分，见 `test/` 目录下的 `expansion.jl`、`determined_prony.jl`、`matrixpencil.jl`、
`lsqexpansion.jl`。
...
| 噪声σ | determined_prony（确定性） | overdetermined_prony | matrix_pencil |
|---|---|---|---|
...
| N | determined_prony | overdetermined_prony | matrix_pencil |
|---|---|---|---|
...
- **无噪、复数、精确**数据下，`determined_prony`、`overdetermined_prony`、`matrix_pencil` 都接近机器精度。
- **抗噪**：确定性 `determined_prony` 在 σ=1e-6 时即数值发散（相对误差爆到 10¹²⁷），只适合无噪声精确数据；
  `overdetermined_prony` 与 `matrix_pencil` 稳健，误差基本等于噪声水平。
- **效率**：
  - `determined_prony` 只用到前 2p 个点，耗时与 `N` 无关（恒 ~5µs），最快；
  - `overdetermined_prony` 随 `N` 近似线性增长；
  - `matrix_pencil` 需对约 `(N/2)²` 的 Hankel 矩阵做 SVD，大致随 `N` 立方增长，`N=2000` 时超过 0.6 s。
- **取舍建议**：追求抗噪稳健用 `matrix_pencil`；无噪小数据图快用 `determined_prony`；
  含噪场景下 `overdetermined_prony` 在强度与速度间折中最佳；对复数序列可进一步用
  `LeastSquareProny` 以梯度精化获得更高的拟合精度。