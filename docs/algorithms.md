...
所有算法都继承抽象类型 `ExponentialExpansionAlgorithm`。每个具体算法只需实现一个
**单阶拟合**方法 `exponential_expansion_n(f, p, alg)`（把数据 `f` 拟合成 `p` 项指数和），
即可接入公共的逐阶迭代流程。
...
`exponential_expansion(f, alg)` 内部按以下流程工作（[`_exponential_expansion_impl`](../src/expansion.jl)）：

1. 校验 `length(f) > 1`；
2. 计算**实际容差** `tol_actual = alg.tol * norm(f)`（相对 L2 容差；详见 [性能分析](performance.md)）；
3. 最大迭代项数 `nitr = min(alg.n, div(L, 2))`；
4. 从 `n = 1` 开始，每轮调用 `exponential_expansion_n(f, n, alg)` 做 `n` 项拟合，
   计算重建误差 `err = expansion_error(f, xs, lambdas)`：
...
- **`stepsize = s > 1`**：先按均匀间隔抽样 `g(n) = f(1 + (n-1)s)`，在降采样序列 `g` 上完成
  拟合后，再通过 [`expansion_changestepsize!`](../src/expansion.jl) 把系数与基底换算回原始采样。
...
- **`stepsize === nothing`**：自动选择步长。算法基于数据**首周期**
  （[`first_period`](../src/expansion.jl)）推导一组候选步长
  `round(period · (0.2, 0.3, 0.35, 0.4, 0.45))` 加上 `1`，逐个尝试，保留重建误差最小者，
  最后再调用 [`cut`](../src/expansion.jl) 按幅值裁剪冗余项。
...
```julia
xs, lambdas = exponential_expansion(f, OverDeterminedProny(stepsize=3))   # 固定步长 3
xs, lambdas = exponential_expansion(f, OverDeterminedProny(stepsize=nothing)) # 自动选步长
```
...
- 优点：实现最简单、只用到前 `2p` 个点，耗时与数据长度 `N` 无关；
- 缺点：**数值上极不稳定、对噪声极其敏感**（σ = 1e-6 时即发散到 ~10¹²⁷），对非指数和
  形式的函数也会在加密网格时失效。**仅作为简单测试用例使用**，实际问题请勿选用。
...
先以 `overdetermined_prony` 得到初猜，再把它重参数化为**实数**未知量
