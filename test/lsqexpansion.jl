# ==========================================================================
# Automatic-step (stepsize=nothing) and LeastSquareProny tests
# ==========================================================================

@testset "exponential_expansion: automatic stepsize (real data)" begin
    atol = 1.0e-8
    for alg in (OverDeterminedProny(n=10, tol=atol, stepsize=nothing),
                MatrixPencil(n=10, tol=atol, stepsize=nothing))
...
@testset "exponential_expansion: automatic stepsize (complex data)" begin
    atol = 1.0e-8
    ydata = complex_exp_data()
    for alg in (OverDeterminedProny(n=10, tol=atol, stepsize=nothing),
                MatrixPencil(n=10, tol=atol, stepsize=nothing),
                LeastSquareProny(n=10, tol=atol, stepsize=nothing))
        xs, lambdas = exponential_expansion(ydata, alg)
        @test expansion_error(ydata, xs, lambdas) / norm(ydata) < atol
    end
end
